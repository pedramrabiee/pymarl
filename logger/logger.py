...
class Logger:
    ...
    def _initialize_tabular(self):
        ...
        csv_file_names = ['episode', 'iteration', 'evaluation']
    ...
    def push_tabular(self, data, cat_key):
        """use this when you have a list of dictionary to be added to _tabular_queue for later use dump_csv"""
        self._tabular_queue[cat_key] = data

    def add_tabular(self, data_dict, cat_key='iteration', stats=False, agent=None, for_csv=False):
        if stats:
            for k, v in data_dict.items():
                stats = self.add_stats(k, v)
                self._log_tabular(stats, cat_key, agent)
        else:
            self._log_tabular(data_dict, cat_key, agent)

    def _log_tabular(self, data_dict, cat_key, agent=None):
        for k, v in data_dict.items():
            if agent is None:
                if len(v) > 1:
                    for i in range(len(v)):
                        keystr = k + "_Agent_%d" % (i+1)
                        self._tabular_queue[cat_key][keystr] = v[i]
                elif len(v) == 1:
                    self._tabular_queue[cat_key][k] = v
            else:
                keystr = k + "_Agent_%d" % agent
                self._tabular_queue[cat_key][keystr] = v
